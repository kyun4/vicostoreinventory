<script setup>
    ...
    import { Chart as ChartJS, Title, BarElement, LinearScale, CategoryScale,Tooltip, Legend, LineElement, PieController, PointElement, ArcElement} from 'chart.js'
    import LogoutModal from '../modals/logout.vue'

    ChartJS.register(Title, BarElement,LinearScale, CategoryScale,  Tooltip,  Legend, LineElement, PieController, PointElement, ArcElement)

    const chartData = {
        labels: ['Jan','Feb','Mar'],
        datasets: [{label: 'Sales', data:[40,456,10], backgroundColor: '#60A5FA'}]
    } // chartData

    const chartOptions = {responsive: true, maintainAspectRatio: false, plugins: {legend: {position: 'top'}, title: {display: true, text: 'Monthly Sales'}}}


    const pieData = {
        labels: ['Vegetables','Drinks','Condiments'],
        datasets: [{
            data: [155,900,90],
            backgroundColor: ['#FA7171','#60A5FA','#FACC15'],
            hoverOffset: 10
        }]
    } // pieData

    const pieDataChartOptions = {
        responsive: true,
        plugins: {
            legend: {position: 'bottom'},
            title: {display: true, text: 'Category Supplies'}
        }
    } // pieDataChartOptions
    ...
    const lineDataChartOptions = {

        responsive: true,
        plugins: {
            legend: { position: 'top'},
            title: {display: true, text: 'Weekly Visitors'}
        },
        scales: {
            y: {beginAtZero: true}
        }

    } // lineDataChartOptions
       ...
       <div :class="['', sidebarOpen ? 'col-span-10' : 'col-span-11']">
            ...
            <section class = "p-7">
                ...
                <div class="grid grid-cols-12">
                    ...
                    <div class="col-span-6 max-lg:col-span-12">

                        <div class="shadow-lg rounded-xl p-4 w-full max-w-md mx-auto bg-white border-2 border-blue-400">

                            <Bar :data = "chartData" :options = "chartOptions"/>

                        </div>

                        <div class="shadow-lg rounded-xl mt-5 p-4 w-full max-w-md mx-auto bg-white border-2 border-green-400">

                            <Line :data = "lineData" :options = "lineDataChartOptions"/>

                        </div>


                    </div> <!-- cols-span-4 Bar Chart -->

                    <div class="col-span-6 max-lg:col-span-12 max-lg:mt-5">

                       <div class="shadow-lg rounded-xl p-4 w-full max-w-md h-50 mx-auto bg-white border-2 border-yellow-400">

                            <Pie :data = "pieData" :options = "pieDataChartOptions"/>
